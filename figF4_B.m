...
% Input variables
%--------------------------------------------------------------------------
allf = linspace(0,7,1000);  % vertical force 
allmub = [12 2 -8];         % effective chemical potential
...
for jj = 1:length(allmub)
    ...
    yvar = cat(2,steadystates(jj,1,2,1),steadystates(jj,:,2,1),steadystates(jj,end,2,1));
    nonzero = yvar>0;
...
% Determine legend
mylegend = cell(1,3*length(allmub));
for ii = 1:length(allmub)
    mylegend{3*ii-2} = ['$\mu_b$= ' num2str(allmub(ii))];
    mylegend{3*ii-1} = '';
    mylegend{3*ii} = '';
end

legend(mylegend,'Location','southeast','Interpreter','latex')
xlabel('$f~(pN)$','Interpreter','latex')
ylabel('$\phi_b$','Interpreter','latex')  
box on
axis([0 allf(end) -0.05 1.05])
set(gca,'FontSize',11)
